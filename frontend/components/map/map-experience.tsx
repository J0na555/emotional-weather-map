...
type LayerKey = 'mood' | 'stress' | 'energy' | 'burnout' | 'sleep'
...
// Real Addis Ababa neighborhoods at true coordinates [lat, lng].
...
// score → emotional state + color for a given layer.
function state(score: number, kind: 'pos' | 'neg') {
  // pos layers (mood/energy/sleep): high score = good. neg (stress/burnout): high = bad.
...
// per-layer score per place
const SCORES: Record<LayerKey, Record<string, number>> = {
...
function buildDistricts(layer: LayerKey): District[] {
  const kind = LAYER_KIND[layer]
  return PLACES.map((p) => {
    const score = SCORES[layer][p.id]
...
const SUMMARIES: Record<LayerKey, string> = {
  mood: 'The city leans calm and hopeful today. Bole and Kazanchis feel notably brighter, while Merkato carries a heavier tone — likely tied to a busy market week.',
  stress: 'Stress is concentrated in the dense western core. Mexico and Merkato are running hot; consider lighter scheduling and visible green spaces this week.',
  energy: 'Energy is high across the north and east. Piassa and Bole are vibrant, while Merkato shows fatigue building toward the weekend.',
  burnout: 'Two early-warning zones detected. Kazanchis and Merkato show sustained depletion — a gentle intervention window opens in the next 5 days.',
  sleep: 'Rest is healthiest in residential Bole and CMC. Central districts report shorter, lighter sleep, tracking closely with elevated stress.',
}

export function MapExperience() {
  const [layer, setLayer] = useState<LayerKey>('mood')
  const active = LAYERS.find((l) => l.key === layer)!
  const districts = buildDistricts(layer)
          ...
          <div className="grid gap-5 md:grid-cols-[1.4fr_1fr]">
            <div className="rounded-3xl border border-border/70 bg-card p-6">
              ...
              <div className="mt-5 flex items-center gap-2 text-xs text-muted-foreground">
                ...
                Updated 2 minutes ago · forecast confidence 86%
              </div>
            </div>
            ...
            <div className="rounded-3xl border border-border/70 bg-card p-6">
              ...
              <p className="mt-2 font-serif text-5xl text-foreground">72</p>
              <p className="mt-1 text-xs text-muted-foreground">
                out of 100 · up 4 this week
              </p>
